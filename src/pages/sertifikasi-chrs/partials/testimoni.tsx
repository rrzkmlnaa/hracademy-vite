...
const mobileData = [
  ...
  {
    videoSrc: "https://www.youtube.com/watch?v=k4W1F9NW4pc",
    thumbnail: " ",
  },
];
  ...
  const moveSlide = (direction: number) => {
    if (sliderRef.current) {
      ...
      const currentTransform = slider.style.transform
        ? parseInt(slider.style.transform.split("(")[1].split("px")[0])
        : 0;
  ...
  <section className="md:hidden w-full">
    <div className="flex flex-col">
      {mobileData.map((video, index) => {
        const isYouTube =
          video.videoSrc.includes("youtube.com") ||
          video.videoSrc.includes("youtu.be");
        const videoId = isYouTube
          ? new URL(video.videoSrc).searchParams.get("v")
          : "";
